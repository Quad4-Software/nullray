...
| Skills / tools / MCP | loaded tables | skills_destroy, tools.registry_destroy, mcp.registry_destroy |
...
Permanent (default allocator): App (owns provider, tools, mcp registries), Session, Provider.

## Destroy pairs

Pair every _init / make_* with destroy. app_destroy chains provider, session, mcp, and tools registries. Job and improve paths that clone a Provider must provider_destroy that copy.
