...
| Toolchain RW (`NULLRAY_TOOLCHAIN`, default on) | Auto RW for Go/Cargo/npm caches under home; `run_shell` forces abs `GOMODCACHE`/`GOCACHE`/`GOPATH` and sandbox `TMPDIR` | Paths recorded, no Landlock | Paths recorded, no Landlock |
