...
static int ssl_tls13_reset_key_share(mbedtls_ssl_context *ssl)
{
    ...
    if (mbedtls_ssl_tls13_named_group_is_x25519mlkem768(group_id)) {
...
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_X25519MLKEM768)
        if (mbedtls_ssl_tls13_named_group_is_x25519mlkem768(*group_list)) {
            *group_id = *group_list;
            return 0;
        }
#endif
                                         ...
                                         size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    if (!mbedtls_ssl_tls13_named_group_is_ecdhe(group_id) &&
        !mbedtls_ssl_tls13_named_group_is_ffdh(group_id) &&
        !mbedtls_ssl_tls13_named_group_is_x25519mlkem768(group_id)) {
    ...
    if (mbedtls_ssl_tls13_named_group_is_x25519mlkem768(group_id)) {
        ...
        MBEDTLS_SSL_CHK_BUF_PTR(p, end, 4);
        p += 4;
        ret = nullray_tls_pq_write_client_share(ssl, p, end, &key_exchange_len);
        if (ret != 0) {
            return ret;
        }
        p += key_exchange_len;
...
#endif /* MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_SOME_EPHEMERAL_ENABLED */
#if defined(MBEDTLS_SSL_TLS1_3_KEY_EXCHANGE_MODE_EPHEMERAL_X25519MLKEM768)
    if (mbedtls_ssl_tls13_named_group_is_x25519mlkem768(group)) {
        ret = nullray_tls_pq_read_server_share(ssl, p, (size_t) (end - p));
        if (ret != 0) {
            return ret;
        }
    } else
