...
static int ssl_tls13_key_schedule_stage_handshake(mbedtls_ssl_context *ssl)
{
    ...
    if (mbedtls_ssl_tls13_key_exchange_mode_with_ephemeral(ssl)) {
        ...
        } else if (mbedtls_ssl_tls13_named_group_is_x25519mlkem768(
                       handshake->offered_group_id)) {
            ret = nullray_tls_pq_compute_shared_secret(
                ssl, &shared_secret, &shared_secret_len);
            if (ret != 0) {
                MBEDTLS_SSL_DEBUG_RET(1, "nullray_tls_pq_compute_shared_secret", ret);
                goto cleanup;
            }
#endif
        } else {
